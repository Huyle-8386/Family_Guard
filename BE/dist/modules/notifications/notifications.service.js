...
class NotificationsService {
    ...
    async respond(uid, notificationId, action) {
        ...
        const { data: relationship, error: relationshipFetchError } = await supabase_1.supabaseAdmin
            .from('relationship')
            .select('*')
        ...
        const { error: relationshipError } = await supabase_1.supabaseAdmin
            .from('relationship')
        ...
        if (action === 'xacnhan') {
            const { error: reverseInsertError } = await supabase_1.supabaseAdmin
                .from('relationship')
                .upsert({
        ...
        if (action === 'xacnhan') {
            try {
                ...
                const responderRelation = this.normalizeRelationLabel(relationship.relation_type);
                await supabase_1.supabaseAdmin.from('notification').insert({
        ...
        const { data: updatedNotification, error: updateNotificationError } = await supabase_1.supabaseAdmin
            .from('notification')
