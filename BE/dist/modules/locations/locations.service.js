...
const safe_zones_service_1 = require("../safe-zones/safe-zones.service");
class LocationsService {
    async updateMyLocation(uid, input) {
        ...
        await supabase_1.supabaseAdmin.from('location_history').insert({
            uid,
            latitude: input.latitude,
            longitude: input.longitude,
            accuracy: input.accuracy ?? null,
        ...
        await safe_zones_service_1.safeZonesService.checkLocationForUser(uid, input.latitude, input.longitude);
        return data;
    }
    async getMyLocation(uid) {
        const { data, error } = await supabase_1.supabaseAdmin
            .from('user_locations')
            .select('*')
            .eq('uid', uid)
    ...
    async getFamilyLocations(currentUid) {
        const { data: relationships, error: relationshipError } = await supabase_1.supabaseAdmin
            .from('relationship')
        ...
        const { data: locations, error: locationError } = await supabase_1.supabaseAdmin
            .from('user_locations')
            .select('*')
            .in('uid', ids);
        if (locationError) {
        ...
        for (const location of locations ?? []) {
            locationMap.set(location.uid, location);
        }
