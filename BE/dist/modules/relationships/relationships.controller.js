...
const relationshipsService = new relationships_service_1.RelationshipsService();
class RelationshipsController {
    async invite(req, res) {
        try {
            const parsed = relationships_validation_1.inviteRelationshipSchema.safeParse(req.body);
            if (!parsed.success) {
            ...
            const data = await relationshipsService.invite(req.userId, parsed.data.target_uid, parsed.data.relation_type, parsed.data.reverse_relation_type);
    ...
    async update(req, res) {
        try {
            ...
            const parsed = relationships_validation_1.updateRelationshipSchema.safeParse(req.body);
            if (!parsed.success) {
                return res.status(400).json({
                    message: 'Dữ liệu không hợp lệ',
                    errors: parsed.error.flatten(),
            ...
            const data = await relationshipsService.updateRelationship(req.userId, relationshipId, parsed.data.relation_type, parsed.data.reverse_relation_type);
            return res.status(200).json({
    ...
    async delete(req, res) {
        try {
            ...
            const data = await relationshipsService.deleteRelationship(req.userId, relationshipId);
            return res.status(200).json({
