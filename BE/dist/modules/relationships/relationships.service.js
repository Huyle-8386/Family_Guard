...
class RelationshipsService {
    ...
    async createRelationshipNotification(params) {
        const { error } = await supabase_1.supabaseAdmin.from('notification').insert({
    ...
    async invite(uid, targetUid, relationType, reverseRelationType) {
        if (uid === targetUid) {
            throw new Error('Không thể tự thêm chính mình');
        }
        const { data: existed, error: existedError } = await supabase_1.supabaseAdmin
            .from('relationship')
            .select('id, processing')
            .or(`and(uid.eq.${uid},relation_id.eq.${targetUid}),and(uid.eq.${targetUid},relation_id.eq.${uid})`)
        ...
        const { data: archivedRelationship, error: archivedRelationshipError } = await supabase_1.supabaseAdmin
            .from('relationship')
            .select('id, processing')
            .eq('uid', uid)
            ...
            .maybeSingle();
        if (archivedRelationshipError)
            throw archivedRelationshipError;
        let relationship;
        let relationshipError;
        if (archivedRelationship) {
            const result = await supabase_1.supabaseAdmin
                .from('relationship')
                .update({
                uid,
        ...
        const inviterRelation = reverseRelationType?.toString().trim() || 'Người thân';
    ...
    async updateRelationship(currentUid, relationshipId, relationType, reverseRelationType) {
        const { data: relationship, error: findError } = await supabase_1.supabaseAdmin
            .from('relationship')
            .select('*')
        ...
        if (relationship.processing === 'xacnhan') {
            const { error: reverseError } = await supabase_1.supabaseAdmin
                .from('relationship')
                .update({
                relation_type: reverseRelationType ?? relationship.reverse_relation_type,
        ...
        try {
            ...
            let relationBefore = this.normalizeRelationLabel(previousReverseRelationType ?? previousRelationType);
            let relationAfter = this.normalizeRelationLabel(reverseRelationType ?? previousReverseRelationType ?? relationType);
            ...
            const content = relationBefore === relationAfter
    ...
    async deleteRelationship(currentUid, relationshipId) {
        const { data: relationship, error: findError } = await supabase_1.supabaseAdmin
            .from('relationship')
            .select('*')
            .eq('id', relationshipId)
        ...
        try {
            const actorName = await this.getUserDisplayName(currentUid);
            const actorRelation = this.normalizeRelationLabel(relationship.reverse_relation_type ?? relationship.relation_type);
            await this.createRelationshipNotification({
                recipientUid: relationship.relation_id,
                relationshipId: relationship.id,
